...
- `publishedAt = null` означает черновик. Публичные запросы **всегда** фильтруют
  `publishedAt != null` и `status != HIDDEN`. Вынеси это в хелперы
  `publicTitleWhere()` / `publicEpisodeWhere()` и используй только их.
- `Title.status = HIDDEN` — рубильник по жалобе правообладателя. Скрывает тайтл
  и все его серии, ничего не удаляя.
