...
- Короткие запросы («нару») триграммами берутся плохо, поэтому к счёту добавлен
  `ILIKE '%запрос%'` по тем же полям. Спецсимволы `%`, `_` и `\` экранируются.
...
- GIN-индексы `gin_trgm_ops` на `nameRu` и `name` созданы, но планом пока не
  используются: счёт считается выражением, а не оператором `%`. На нынешнем
  объёме это доли миллисекунды; переход на `%` с порогом — в открытых вопросах
  роадмапа.
